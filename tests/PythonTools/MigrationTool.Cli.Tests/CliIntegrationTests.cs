...
public class CliIntegrationTests : IDisposable
{
    ...
    public CliIntegrationTests()
    {
        ...
        // Test runs from: tools/tests/PythonTools/MigrationTool.Cli.Tests/bin/Debug/net9.0
        // CLI is at: tools/src/PythonTools/MigrationTool.Cli
        var testDir = AppContext.BaseDirectory;
        _cliProjectPath = Path.GetFullPath(Path.Combine(
            testDir, 
            "..", "..", "..", // bin/Debug/net9.0
            "..", "..", "..", // tests/PythonTools/MigrationTool.Cli.Tests
            "src", "PythonTools", "MigrationTool.Cli"
        ));
    }
    ...
    [Fact]
    public async Task UpdateSolution_UpdatesProjectPath()
    {
        // Arrange - create a simple solution with known paths
        var solutionContent = """
            Microsoft Visual Studio Solution File, Format Version 12.00
            Project("{FAE04EC0-301F-11D3-BF4B-00C04F79EFBC}") = "MyProject", "src\MyProject\MyProject.csproj", "{12345678-1234-1234-1234-123456789012}"
            EndProject
        ...
        slnContent.Should().Contain(@"lib\MyProject\MyProject.csproj");
        slnContent.Should().NotContain(@"src\MyProject\MyProject.csproj");
    }
